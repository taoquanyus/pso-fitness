...
for p=1:length(t)
    ...
    if(p>0&&p<20)                               %%%%定义阶梯流量%%%%%
        ...
        %B=alpha(1)^(Qp(1)*heaviside(Ql(1)))*((l-d)/M)^P*C;
        %B=alpha(1)^(Q(1)*sign(Q(1)))*((l-d)/M)^P*C
        B=(Qp(1)*sign(Qp(1)))^alpha(1)*((l-d)/M)^P*C;
%         B=(Qp(1)*sign(Qp(1)))^alpha*((l-d)/M)^P*C;
    ...
    elseif(p>=20&&p<39)
        ...
        %B=Ql(2)^(alpha(2)*heaviside(Ql(2)))*((l-d)/M)^P*C;
        
        %B=alpha(2)^(Qp(2)*heaviside(Ql(2)))*((l-d)/M)^P*C;
        B=(Qp(2)*sign(Qp(2)))^alpha(1)*((l-d)/M)^P*C;
%          B=(Qp(2)*sign(Qp(2)))^alpha*((l-d)/M)^P*C;
        sl(p)=B*(Q(2)^P);
        sa(p)=mean(sz);
        s(p)=sa(p)+sl(p);
    elseif(p>=39&&p<58)
        ...
        %B=alpha(3)^(Qp(3)*heaviside(Ql(3)))*((l-d)/M)^P*C;
        
        B=(Qp(3)*sign(Qp(3)))^alpha(1)*((l-d)/M)^P*C;
%  B=(Qp(2)*sign(Qp(2)))^alpha*((l-d)/M)^P*C;
        sl(p)=B*(Q(3)^P);
        sa(p)=mean(sz);
        s(p)=sa(p)+sl(p);
    elseif(p>=58&&p<77)
        ...
        %B=alpha(4)^(Qp(4)*heaviside(Ql(4)))*((l-d)/M)^P*C;
        
        B=(Qp(4)*sign(Qp(4)))^alpha(1)*((l-d)/M)^P*C;
%  B=(Qp(2)*sign(Qp(2)))^alpha*((l-d)/M)^P*C;
        sl(p)=B*(Q(4)^P);
        sa(p)=mean(sz);
        s(p)=sa(p)+sl(p);
    elseif(p>=77)
        ...
        %B=alpha(5)^(Qp(5)*heaviside(Ql(5)))*((l-d)/M)^P*C;
        
        B=(Qp(5)*sign(Qp(5)))^alpha(1)*((l-d)/M)^P*C;
%  B=(Qp(2)*sign(Qp(2)))^alpha*((l-d)/M)^P*C;
        sl(p)=B*(Q(5)^P);
        sa(p)=mean(sz);
        s(p)=sa(p)+sl(p);
...
% % %             Sum3=(Qp(3)/(4*pi*T))*(W_ur3+Sum3);
% % %             Sum4=(Qp(4)/(4*pi*T))*(W_ur4+Sum4);
% % %             s(p)=Sum1+Sum2+Sum3+Sum4;     %%%%第二阶梯阶段降深+第一阶梯降深%%%%%
% % %             sz(m)=s(p);
% % %         end
% % %         s(p)=mean(sz);
% % %     elseif(p>=77)
...
% % %             Sum4=(Qp(4)/(4*pi*T))*(W_ur4+Sum4);
% % %             Sum5=(Qp(5)/(4*pi*T))*(W_ur5+Sum5);
% % %             s(p)=Sum1+Sum2+Sum3+Sum4+Sum5;     %%%%第二阶梯阶段降深+第一阶梯降深%%%%%
% % %             sz(m)=s(p);
% % %         end
% % %         s(p)=mean(sz);
% % %     end
...
% %             sz(m)=sa(p);
% %         end
% %         B=(-1)^heaviside(Ql(1))*C*(alpha*(l-d)/M)^P;
% %         sl(p)=B*(Q(1)^P);
% %         sa(p)=mean(sz);
% %         s(p)=sa(p)+sl(p);
% %     elseif(p>=20&&p<39)
...
% %             sz(m)=sa(p);
% %         end
% %         B=(-1)^heaviside(Ql(2))*C*(alpha*(l-d)/M)^P;
% %         sl(p)=B*(Q(2)^P);
% %         sa(p)=mean(sz);
% %         s(p)=sa(p)+sl(p);
% %     elseif(p>=39&&p<58)
...
% %             sz(m)=sa(p);
% %         end
% %         B=(-1)^heaviside(Ql(3))*C*(alpha*(l-d)/M)^P;
% %         sl(p)=B*(Q(3)^P);
% %         sa(p)=mean(sz);
% %         s(p)=sa(p)+sl(p);
% %     elseif(p>=58&&p<77)
...
% %             sz(m)=sa(p);
% %         end
% %         B=(-1)^heaviside(Ql(4))*C*(alpha*(l-d)/M)^P;
% %         sl(p)=B*(Q(4)^P);
% %         sa(p)=mean(sz);
% %         s(p)=sa(p)+sl(p);
% %     elseif(p>=77)
...
% %             sz(m)=sa(p);
% %         end
% %         B=(-1)^heaviside(Ql(5))*C*(alpha*(l-d)/M)^P;
% %         sl(p)=B*(Q(5)^P);
% %         sa(p)=mean(sz);
% %         s(p)=sa(p)+sl(p);
% %     end
